...
- Tasks:
  ...
  - `DOC-001G` Document tests with intent-focused docstrings for fixtures/helpers and non-obvious scenarios. Status: `in_progress` (`conftest`, `test_ui`, `test_player_service`, `test_playlist_store`, `test_playlist_viewport`, `test_metadata_service`, `test_audio_envelope_analysis`, `test_visualizer_registry`, `test_visualizer_host`, `test_visualizer_cover_ascii`, `test_focus_navigation_matrix`, `test_startup_resilience`, `test_visualizer_selection_integration`, `test_app_envelope_analysis`, `test_non_blocking_paths`, `test_performance_opt_in`, `test_logging_config`, `test_metadata_debounce`, `test_audio_level_service`, `test_vlc_backend_unit` documented; remaining test files pending)
